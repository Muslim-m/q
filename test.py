...
df = pd.read_excel('data/table.xlsx', index_col=False)
...
with pd.ExcelWriter('Report2.xlsx', engine='xlsxwriter') as writer:
    ...
    worksheet.merge_range('F3:K4', "30.10.2025 11:19", normal_center)
    ...
    for row_idx, row_data in final_result.iterrows():
        for col_idx, value in enumerate(row_data):
            ...
            if col_idx == 4 and 1 <= row_idx <= 13 and e8_reference_value is not None:
                # Try to convert value to float and check if < E8 reference value
                try:
                    if pd.notna(value) and float(value) < e8_reference_value:
...
print("✅ Report2.xlsx fayli yaratildi.")